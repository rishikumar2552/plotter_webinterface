...
$(document).ready(function() {
  ...
  //  Keep track of websocket server status
	...
	socket.on('ports', function (data) {
		...
		for (var i=0; i<data.length && data[i]!=undefined ; i++) {
			$('#choosePort').append('<option value="'+i+'">'+data[i].comName+':  '+data[i].manufacturer+'</option>');
	console.log("inside .on ports "+data[i]); 	
}
	...
	console.log("inside qstatus "+ data );	
	...
	console.log("handing serial read event");
	...
	$('#choosePort').on('change', function() {
		// select port
                     var tmp= $('#choosePort').val();
                    tmp = +tmp;
		socket.emit('usePort',tmp);
	});
